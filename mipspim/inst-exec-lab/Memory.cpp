...
uint8_t* Memory::getMemoryPtr(unsigned addr) {
	unsigned x; //integer to determine offset
	uint8_t* ptr; //pointer to be ultimately returned
	if ((addr >= stackBase) && (addr < stackBound)) { //if in stack
		x = (addr-stackBase);  //address offset
		ptr = &stackMemory[x];  //return address of variable
	//	cout << "stack" << endl;  --testing for location
	} else if ((addr >= dataBase) && (addr < dataBound)) { //if in data
		x = (addr-dataBase); 
		ptr = &dataMemory[x];
	//	cout << "data" << endl; --testing for location
	} else if ((addr >= textBase) && (addr < textBound)) { //if in text
		x = (addr-textBase);
		ptr = &textMemory[x];
	//	cout << "text" << endl; --testing for location
	} else {
		assert(false && "memory access out of range"); //kills program if not in range
	}
