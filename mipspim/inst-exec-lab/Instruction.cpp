...
//Below block commented out for Lab 7 - Instruction Execution
...
unsigned Instruction::signExtend8(unsigned n) {
	int neg = 0x00000080; //tests if MSB is negative
	if (neg & n) { //if negative; bitwise test
		n += 0xFFFFFF00; //make remainder of bits negative, extending sign
	}		
	return n;
}
...
unsigned Instruction::signExtend16(unsigned n) {
	int neg = 0x00008000; //tests if MSB is negative
	if (neg & n) { //if negative; bitwise test
		n += 0xFFFF0000; //make remainder of bits negative, extending sing
	}
	return n;
}
...
bool Instruction::exec() {
  ...
  case SLTI:
    ...
    break;
  case SLTIU:
	reg[rt] = ((int) reg[rs] < signExtend16(imm)) ? 1 : 0;
    break;
  case ANDI:
	reg[rt] = reg[rs] & imm;
    break;
